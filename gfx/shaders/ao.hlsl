...
/*
	...
	float linear_depth = g_proj_constants[1] / (depth - g_proj_constants[0]);
	if(depth == 1) linear_depth = 0;
	float3 vs_pos = IN.viewspace_ray * linear_depth;

	float occlusion = 0;
		
	float3 normal = g_normal.Load(pix_coord);
	for(int i = -6; i < 7; i+=2)
	{
		for(int j = -6; j < 7; j+=2)
		{
			int3 sample_coord = int3(IN.position.xy + int2(i, j), 0);
		
			float3 sample_normal = g_normal.Load(sample_coord).xyz;	
			float sample_depth = g_depth.Load(sample_coord);
	

			float depth_diff = abs(sample_depth - depth);
			
			float sample_depth_disc = lerp(0, 1, depth_diff / 0.00015);
			float sample_occlusion = sample_depth_disc;
			if(depth_diff > 0.00016)
			{ 
				sample_occlusion = 0;
			}
	...
	return (1 - occlusion) * float4(albedo, 1);
*/