...
#define PASSN_X_SIZE 16
#define PASSN_Y_SIZE 16
groupshared float passn_gsmem[3][PASSN_Y_SIZE][PASSN_X_SIZE*2]; //for depth,color

[numthreads(PASSN_X_SIZE, PASSN_Y_SIZE, 1)]
...
[numthreads(PASS0_X_SIZE, PASS0_Y_SIZE, 2)]
void csPass2HFirstPass(uint3 Gid : SV_GroupId, uint3 GTid : SV_GroupThreadID, uint3 DTid : SV_DispatchThreadID)
{
	...
	for(int i = 0; i < 2; i++)
	{
		int2 offset = int2((2 * i + DTid.z) * PASS0_X_SIZE + GTid.x, GTid.y);
		int2 outXY = xy0 + offset;
		int2 inIJ = offset;
		//write
